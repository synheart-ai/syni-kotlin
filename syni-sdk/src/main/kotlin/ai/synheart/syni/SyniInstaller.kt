...
class SyniInstaller(
    ...
    /**
     * Total on-disk bytes occupied by [spec]'s model files (the GGUF plus
     * its sibling `tokenizer.json`). Returns 0 when nothing is installed.
     * Useful for telling the user how much storage a delete will reclaim.
     */
    fun installedBytes(spec: SyniModelSpec): Long {
        val dir = modelsDir()
        var total = 0L
        for (f in listOf(File(dir, spec.filename), File(dir, "tokenizer.json"))) {
    ...
    /**
     * Permanently deletes [spec]'s model files from disk — the GGUF and
     * its sibling `tokenizer.json` — reclaiming the ~1.1 GB the install
     * occupies.
     *
     * Returns the number of bytes actually freed. Idempotent: deleting
     * when nothing is on disk is a no-op that returns 0. Throws
     * [SyniInstallException] if a file exists but cannot be removed.
     */
    fun deleteModel(spec: SyniModelSpec): Long {
        val dir = modelsDir()
        val freed = installedBytes(spec)
        for (f in listOf(File(dir, spec.filename), File(dir, "tokenizer.json"))) {
            if (f.exists() && !f.delete()) {
                throw SyniInstallException(
                    "failed to delete model file ${f.absolutePath}"
                )
            }
        }
        return freed
